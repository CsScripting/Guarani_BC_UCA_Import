	...
	try:
		...
		if (validacion == True):
			...
			df_events = df_events [[v_comision, v_subcomision, v_day, v_hour_begin,
									...
									g_plan_cod, v_mod_name,v_classrooms_code_guarani]]
...
def check_nulls(df):
	...
	df = df [[v_comision, v_subcomision, v_day, v_hour_begin, v_hour_end, 
			  ...
			  v_students, v_group, v_weeks, 
			  v_year,v_course_code, v_codigo_plan, v_classrooms_code_guarani]]
...
def get_name_salas_btt(df_events, df_salas):
	...
	## pass all when only left:

	# df[v_classrooms_type] = np.where(df['_merge'] != 'both'  , '3', df[v_classrooms_type])
	# df[s_name_btt] = np.where(df['_merge'] != 'both'  , 'SinAula', df[s_name_btt])
...
def join_curriculum (df_events, df_courses, df_groups):
	...
	#Asign Name Comission:

	df_events_map_g[v_comision] = np.where(df_events_map_g[v_subcomision] != '0' ,
								  df_events_map_g[v_comision] + ' - ' + df_events_map_g[v_subcomision],
								  df_events_map_g[v_comision])


	df_events_map_g[v_section] = df_events_map_g[v_comision] + ' ' + df_events_map_g[v_section] 


	df_events_map_g = df_events_map_g [[v_section, v_students, v_hour_begin, v_hour_end, v_day,
										v_mod_name, v_mod_acron, v_mod_cod,
										s_sala_name, s_sala_edificio, v_weeks, 
										v_typology,
										v_bullet_group,v_plan_name, g_plan_cod, v_year,
										c_courses_name, c_courses_sigla, v_course_code]]
...
def grouped_data (df):
	...
	list_series = [v_section, v_students, v_hour_begin, v_hour_end, v_day,
				   v_mod_name, v_mod_acron, v_mod_cod,v_mod_area,
				   s_sala_name, s_sala_edificio, v_weeks, 
				   v_typology]
	

	df.sort_values(by = [v_mod_cod], ascending = False, inplace = True )
...
def insert_name_acad_term (df, academicTerm):
	...
	df.insert(0, v_event_name, df[v_section] + '_' + df[v_mod_name])
